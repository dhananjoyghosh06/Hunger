...
import { adding } from "../store/orderDetails.js";
function MyCart() {
  ...
  axios.post("http://localhost:8000/delhivery/getAddressPayload", { userToken })
    ...
    useEffect(()=>{
      const getUserData = ()=>{
        ...
        .then((p)=>{
          setMobileNo(p.data.payload.mobileNo);
  ...
  return (
    <div className="flex gap-10 bg-gray-100">
              ...
              <div className="flex gap-5 px-4 bg-gray-200" key={item.id}>
                ...
                <div className="flex flex-col gap-8 py-2">
                  <div className="flex flex-col gap-2 ">
                    ...
                    <p className="flex gap-2"><FaMinus className="size-[15px] mt-1" onClick={minusToogle}/> {item.quantity} <FaPlus className="size-[15px] mt-1" onClick={plusToogle}/></p>
                  </div>
                  <button
      ...
      <div className="w-[35%] bg-white flex flex-col h-[100vh]">
        ...
        {items.length >= 1 ? <Link className="h-[40px] w-[100px] bg-orange-500 text-black flex items-center justify-center" onClick={handlePayment}>Payment</Link> : null
}
        </div>
      </div>
    </div>
