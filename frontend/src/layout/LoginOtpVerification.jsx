    ...
    switch (index) {
      ...
      case 3:
        ...
        if(value !==""){
          ...
          axios.post("http://localhost:8000/user/login/otp-verification", {OTP, mobileNo})
          .then((p)=>{
      ...
      <div className="h-[500px] w-[90%] sm:w-[600px] md:w-[400px] bg-white rounded-lg flex flex-col items-center justify-between py-20">
        ...
        <div className="basis-1/3 flex flex-col items-center justify-evenly">
          <p className="text-center text-red-400" >{messages}</p>
              ...
              <h2>Not Recived OTP? <button className={`${buttonclicked ? 'text-gray-400': 'text-red-500'}`} disabled={buttonclicked} onClick={handleOnclick}>Resend Now</button></h2>
        </div>
      </div>
    </div>
  );
